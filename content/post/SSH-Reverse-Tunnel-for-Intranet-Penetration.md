...
lastmod = 2026-01-07T19:14:16+08:00
tags = ["network"]
categories = ["技术"]
draft = false
+++
...
比较方便的是，配置完成后 A 机器不需要 root 权限可以直接修改 `~/.ssh/config` 配置文件，因为它位于本用户 HOME 目录下。改完后，有 root 权限可以直接重启服务，没 root 权限可以在 B 机器上踢线，会重启服务。
