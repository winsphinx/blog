...
lastmod = 2024-10-03T09:48:16+08:00
tags = ["linux", "network"]
categories = ["技术"]
draft = false
+++
...
# 在备份目录上删除旧备份
cd $DEST_DIR && (ls -t | tail -n +8 | xargs --no-run-if-empty rm -rvf)
```
...
# 在远程服务器上删除旧备份
ssh -p $SSH_PORT root@$REMOTE_HOST "cd $DEST_DIR && (ls -t | tail -n +8 | xargs --no-run-if-empty rm -rvf)"
```
